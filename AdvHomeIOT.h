...
class AdvHomeIOT {
  public:
    AdvHomeIOT(const char* token);  // Accept token as constructor input

    void begin();                   // Setup: call in setup()
    void loop();                    // Main loop: call in loop()
    PubSubClient& getClient();      // Expose MQTT client to sketch
  ...
  private:
    ...
    void connectMQTT();
    bool authenticateToken();       // Token-MAC authentication placeholder
    void checkFirmwareUpdate();     // OTA update check and install
};
...
void AdvHomeIOT::begin() {
  ...
  _secureClient.setInsecure();  // Accept all certificates (for dev)
  _client.setServer(mqtt_server, mqtt_port);

  checkFirmwareUpdate();        // Check for OTA updates on boot
}
...
void AdvHomeIOT::checkFirmwareUpdate() {
  ...
  if (httpClient.begin(client, VERSION_CHECK_URL)) {
    ...
    if (httpCode == 200) {
      ...
      if (split > 0) {
        ...
        Serial.println("⚠️ Invalid version file format.");
      }
    } else {
    ...
    Serial.println("❌ Could not connect to version server.");
  }
}
